...
abstract class DeviceServiceProvider<T : BaseDeviceState> {
    ...
    val config: RadarConfiguration
        get() = (radarService?.application as RadarApplication?)?.configuration
    ...
    protected fun configure(bundle: Bundle) {
        ...
        (this.radarService!!.applicationContext as RadarApplication).configureProvider(bundle)
        val permissions = permissionsNeeded
        bundle.putBoolean(NEEDS_BLUETOOTH_KEY, permissions.contains(BLUETOOTH) || permissions.contains(BLUETOOTH_ADMIN))
    }
