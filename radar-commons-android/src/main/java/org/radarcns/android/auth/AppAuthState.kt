...
import org.radarcns.android.auth.LoginManager.Companion.AUTH_TYPE_UNKNOWN
import org.radarcns.android.auth.portal.ManagementPortalClient.Companion.SOURCES_PROPERTY
...
class AppAuthState private constructor(builder: Builder) {
    ...
    fun toIntent(): Intent {
        val intent = Intent()
        intent.putExtras(addToBundle(Bundle()))
        return intent
    }
     ...
     */
    fun addToBundle(bundle: Bundle): Bundle {
        return bundle.apply {
            putString(LOGIN_PROJECT_ID, projectId)
            putString(LOGIN_USER_ID, userId)
            putString(LOGIN_ATTRIBUTES, serializableAttributeList())
            putStringArrayList(LOGIN_APP_SOURCES_LIST, ArrayList<String>(sourceMetadata.size).apply {
                addAll(sourceMetadata.map(SourceMetadata::toJsonString))
            })
            putStringArrayList(LOGIN_SOURCE_TYPES, ArrayList<String>(sourceTypes.size).apply {
    ...
    companion object {
        ...
        fun from(bundle: Bundle): AppAuthState {
            bundle.classLoader = AppAuthState::class.java.classLoader

            return Builder().apply {
                try {
                    bundle.getSerializable(LOGIN_HEADERS)
                            ?.let {
                    ...
                    logger.warn("Cannot set AppAuthState parseHeaders", ex)
                }

                try {
                    parseSourceMetadata(bundle.getStringArrayList(LOGIN_APP_SOURCES_LIST))
                } catch (ex: Exception) {
                    logger.warn("Cannot deserialize source metadata", ex)
                }

                try {
                    parseSourceTypes(bundle.getStringArrayList(LOGIN_SOURCE_TYPES))
                } catch (ex: Exception) {
                    logger.warn("Cannot deserialize source types", ex)
                }
                ...
                isPrivacyPolicyAccepted = bundle.getBoolean(LOGIN_PRIVACY_POLICY_ACCEPTED)
                bundle.getString(LOGIN_ATTRIBUTES)?.also {
                    parseAttributes(it)
                }
                parseHeaders(bundle.getString(LOGIN_HEADERS_LIST))
                authenticationSource = bundle.getString(LOGIN_AUTHENTICATION_SOURCE)
                needsRegisteredSources = bundle.getBoolean(LOGIN_NEEDS_REGISTERD_SOURCES)
            }.build()
        }
