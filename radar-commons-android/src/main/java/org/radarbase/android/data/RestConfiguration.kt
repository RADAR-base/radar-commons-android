...
data class RestConfiguration(
        ...
        fun configure(config: SingleRadarConfiguration) {
                ...
                kafkaConfig = config.optString(RadarConfiguration.KAFKA_REST_PROXY_URL_KEY)?.let {
                        ServerConfig(it).apply {
                                isUnsafe = unsafeConnection
                        }
                }
                schemaRetriever = config.optString(RadarConfiguration.SCHEMA_REGISTRY_URL_KEY)?.let {
