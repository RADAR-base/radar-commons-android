...
class PolarSenseManager(
    polarService: PolarSenseService,
    private val applicationContext: Context
) : AbstractSourceManager<PolarSenseService, PolarSenseState>(polarService) {
    ...
    private var deviceId: String = "D833AF2E"
    ...
    override fun start(acceptableIds: Set<String>) {
        ...
        mHandler.execute {
            wakeLock = (service.getSystemService(POWER_SERVICE) as PowerManager?)?.let { pm ->
                pm.newWakeLock(PowerManager.PARTIAL_WAKE_LOCK, "org.radarcns.polar:PolarSenseManager")
                    .also { it.acquire() }
            }
        }
