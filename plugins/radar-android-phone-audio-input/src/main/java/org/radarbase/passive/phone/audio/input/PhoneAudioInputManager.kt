...
class PhoneAudioInputManager(service: PhoneAudioInputService) :
    ...
    private val tempHandler = SafeHandler.getInstance("Temp", Process.THREAD_PRIORITY_BACKGROUND)
    ...
    override fun start(acceptableIds: Set<String>) {
        ...
        Thread {
            startAudioRecording()
            tempHandler.delay(15000, ::stopRecording)
        }.start()
    }
    ...
    private fun clearAudioDirectory() {
        audioDir?.let { audioDir ->
            ...
            audioDir.walk().filter { it.name.startsWith("phone_audio_input") && it.path.endsWith(".wav") }
                .forEach { it.delete() }
        }
    }
