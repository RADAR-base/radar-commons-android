...
class PhoneAudioInputService: SourceService<PhoneAudioInputState>() {
    ...
    override fun configureSourceManager(
        ...
        manager.recorderBufferSize = config.getInt(PHONE_AUDIO_INPUT_RECORDER_BUFFER_SIZE, PHONE_AUDIO_INPUT_RECORDER_BUFFER_SIZE_DEFAULT)
        manager.currentAudioFormat = config.getInt(PHONE_AUDIO_INPUT_CURRENT_AUDIO_FORMAT, PHONE_AUDIO_INPUT_CURRENT_AUDIO_FORMAT_DEFAULT)
        manager.currentChannel = config.getInt(PHONE_AUDIO_INPUT_CURRENT_CHANNEL, PHONE_AUDIO_INPUT_CURRENT_CHANNEL_DEFAULT)
        manager.currentSampleRate = config.getInt(PHONE_AUDIO_INPUT_CURRENT_SAMPLE_RATE, PHONE_AUDIO_INPUT_CURRENT_SAMPLE_RATE_DEFAULT)
    }
    ...
    companion object {
        ...
        const val PHONE_AUDIO_INPUT_CURRENT_SAMPLE_RATE_DEFAULT = 44100
    }
}