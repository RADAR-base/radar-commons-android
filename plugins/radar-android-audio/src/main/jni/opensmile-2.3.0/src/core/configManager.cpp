      ...
      if (element[i].enabled) {
        ...
        if (texOutput) {
          ...
          if ((element[i].type == CFTP_OBJ)&&(_subtype)) {
            if (element[i].subType != NULL) {
              if (basestr != NULL)
                element[i].subType->printTypeHelp(
                        myvprint("%s.%s%s", basestr, element[i].name, ArrS));
              else
    ...
    for (i=0; i<I; i++) {
      ...
      if ((element[i].enabled)&&(element[i].printDflt)/*&&(element[i].type < CFTP_ARR)*/&&(ok)) {
        ...
        if ((element[i].type == CFTP_OBJ)&&(_subtype)) {
          if (element[i].subType != NULL) {
              if (basestr != NULL)
                  element[i].subType->printTypeDfltConfig(
                          myvprint("%s.%s%s", basestr, element[i].name, ArrS), _subtype,
                          withDescription);
              else
                  element[i].subType->printTypeDfltConfig(myvprint("%s%s", element[i].name, ArrS),
                                                          _subtype, withDescription);
          }
        }
      }
    }
