...
int smilePcm_parseWaveHeader(void *raw, long long N, sWaveParameters *pcmParam)
{
  if ((raw != NULL)&&(pcmParam!=NULL)) {
    ...
    if (nRead != sizeof(sRiffPcmWaveHeader)) {
      printf("smilePcm: Error reading %zd bytes (header) from beginning of wave data! The given array is too short (N=%lli)!",sizeof(sRiffPcmWaveHeader),N);
      return 0;
    }
...
//		(head->Subchunk2ID != 0x61746164) ||
    ...
    while ((head->Subchunk2ID != 0x61746164)&&(safetytimeout>0)) { // 0x61746164 = 'data'
      ...
      if (ptr > N) {
        fprintf(stderr,"smilePcm: less bytes read from wave data than there should be (%zd) while reading sub-chunk header! File seems broken!\n",sizeof(sRiffChunkHeader));
        return 0;
      }
...
int smilePcm_readWaveHeader(FILE *filehandle, sWaveParameters *pcmParam, const char *filename)
{
  ...
  if ((filehandle != NULL)&&(pcmParam!=NULL)) {
    ...
    if (nRead != sizeof(head)) {
      printf("smilePcm: Error reading %zd bytes (header) from beginning of wave file '%s'! File too short??",sizeof(head),filename);
      return 0;
    }
...
//      (head.Subchunk2ID != 0x61746164) ||
    ...
    while ((head.Subchunk2ID != 0x61746164)&&(safetytimeout>0)) { // 0x61746164 = 'data'
      ...
      if (nRead != sizeof(chunkhead)) {
        fprintf(stderr,"smilePcm: less bytes read (%i) from wave file '%s' than there should be (%zd) while reading sub-chunk header! File seems broken!\n",nRead,filename,sizeof(chunkhead));
        return 0;
      }
      head.Subchunk2ID = chunkhead.SubchunkID;
      head.Subchunk2Size = chunkhead.SubchunkSize;
...
void smileHtk_Swap32 ( uint32_t *p )
{
  uint8_t temp,*q;
  q = (uint8_t*) p;
