...
class PolarH10Manager(
    polarService: PolarH10Service,
    private val applicationContext: Context
) : AbstractSourceManager<PolarH10Service, PolarH10State>(polarService) {
    ...
    companion object {
        private const val TAG = "POLAR-H10"
    ...
    override fun start(acceptableIds: Set<String>) {
        ...
        mHandler.execute {
            wakeLock = (service.getSystemService(POWER_SERVICE) as PowerManager?)?.let { pm ->
                pm.newWakeLock(PowerManager.PARTIAL_WAKE_LOCK, "org.radarcns.polar:PolarH10Manager")
                    .also { it.acquire() }
            }
        }
                    ...
                    .subscribe(
                        { polarEcgData: PolarEcgData ->
                            for (data in polarEcgData.samples) {
                                Log.d(TAG, "ECG yV: ${data.voltage} timeStamp: ${data.timeStamp} time: ${PolarH10Utils.convertEpochPolarToUnixEpoch(data.timeStamp)}")
                                send(
                                    ecgTopic,
                                    PolarEcg(
                                        name,
                                        PolarH10Utils.convertEpochPolarToUnixEpoch(data.timeStamp),
                                        getTimeSec(),
                                        data.voltage
                                    )
                                )
