...
public class BinaryMessageDecoder<D> extends MessageDecoder.BaseDecoder<D> {
  private final byte[] headerBuffer = new byte[10];
  private final ByteBuffer fpBuffer = ByteBuffer.wrap(headerBuffer).order(ByteOrder.LITTLE_ENDIAN);
  ...
  public D decode(InputStream stream, D reuse) throws IOException {
    ...
    RawMessageDecoder<D> decoder = getDecoder(fpBuffer.getLong(2));
