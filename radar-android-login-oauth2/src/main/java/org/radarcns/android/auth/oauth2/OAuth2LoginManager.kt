...
class OAuth2LoginManager(private val service: AuthService, private val projectIdClaim: String, private val userIdClaim: String) : LoginManager, LoginListener {
    ...
    override fun start(authState: AppAuthState) {
        val app = service.application as RadarApplication
    ...
    override fun loginSucceeded(manager: LoginManager?, appAuthState: AppAuthState) {
        val token = appAuthState.token
        if (token == null) {
        ...
        try {
            val authState = processJwt(appAuthState, Jwt.parse(token))
            this.service.loginSucceeded(this, authState)
